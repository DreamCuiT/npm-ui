      ...
      <div class="common-table"
        ...
        <el-table ref="table"
          ...
          <template v-for="(item,index) in columns">
            <!-- 列表自定义列 -->
            <el-table-column v-if="item.scopedSlots && item.scopedSlots.customRender == 'custom'"
                             :key="index"
                             :label="item.title"
                             :prop="item.dataIndex"
                             :type="item.type"
            ...
            <el-table-column v-else
                             ...
                             v-bind="item.columnConfig"></el-table-column>
          </template>
          <el-table-column v-if="addRow"
                           prop="detele"
                           label="删除"
                           :width="60"
                           align="center">
  ...
  computed: {
    columnDataIndex () {
      ...
      this.columns.map(item => {
        item.dataIndex && (columnsObj[item.dataIndex] = item.default)
      })
      return columnsObj
    },
    changeParams () {
