<template>
  <div class="right-board">
    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px">
          ...
          <el-form-item v-if="activeData.__vModel__!==undefined" :label="activeData.__config__.selectChildTable?'子表字段':'数据源字段'">
            <el-select
              v-model="activeData.__vModel__"
              :placeholder="activeData.__config__.selectChildTable?'请选择子表字段':'请选择数据源字段'"
              :style="{width: '100%'}"
              @change="formFieldsChange"
              clearable
            >
              <el-option
                v-for="item in activeData.__config__.selectChildTable?childrenFieldsOption:formFieldsOption"
                :key="item.value"
                :label="item.label"
                :value="item.label"
              ></el-option>
          ...
          <el-form-item v-if="activeData.type==='viewField'" label="view变量">
            <el-input
              v-model="activeData.viewField"
              :style="{width: '100%'}"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item v-if="activeData.__config__.variable!==undefined" label="变量">
            <el-input
              v-model="activeData.__config__.variable"
              :style="{width: '100%'}"
              clearable
            ></el-input>
          </el-form-item>
          ...
          <el-form-item v-if="activeData.__config__.layout==='masterSlaveTable'" label="绑定子表">
            <el-select
              v-model="activeData.__config__.childrenTable"
              placeholder="请选择绑定的子表"
              :style="{width: '100%'}"
              @change="childrenTableChange"
              clearable
            >
              <el-option
                v-for="item in childrenTableOption"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              ></el-option>
          ...
          <el-form-item v-if="activeData.__config__.layout==='masterSlaveTable'" label="子表展示模式">
            <el-select
              v-model="activeData.__config__.displayMode"
              placeholder="请选择子表展示模式"
              :style="{width: '100%'}"
              clearable
            >
              <el-option label="单行展示" value="singleLine"></el-option>
              <el-option label="多行展示" value="multiLine"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item v-if="activeData.__config__.fieldType!==undefined && 'p8-upload' === activeData.__config__.tag" label="附件类别">
            <el-input
              v-model="activeData.__config__.fieldType"
              placeholder="请输入附件类别"
              @input="changeRenderKey"
            />
          </el-form-item>
          ...
          <template
            v-if="['eject-select'].indexOf(activeData.__config__.tag) > -1"
          >
            <el-divider>选项</el-divider>
            <el-form-item label="弹出框组件">
              <el-select v-model="activeData.__config__.selectComp" placeholder="请选择弹出框组件" clearable @change="changeEjectSelectComp">
                <el-option v-for="item in selectCompOption" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
              <span class="view-fields-link" title="查看viewFields" @click="viewFields">
                <i class="el-icon-link" />
              </span>
            </el-form-item>
            <el-form-item label="回填name">
              <el-select v-model="activeData.__config__.backfillParam.label" clearable>
                <el-option v-for="item in ejectSelectColumns" :label="item.label" :value="item.value" :key="item.value"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="回填id">
              <el-select v-model="activeData.__config__.backfillParam.value" clearable>
                <el-option v-for="item in ejectSelectColumns" :label="item.label" :value="item.value" :key="item.value"></el-option>
              </el-select>
            </el-form-item>
            <el-input v-show="false" v-model="activeData.__config__.reportCode"></el-input>
            <el-divider/>
          </template>
          ...
          <el-form-item v-if="activeData.accept !== undefined" label="文件上传类型">
            <el-input v-model="activeData.accept"
              placeholder="请填写(以','拼接: .png,.txt,.zip)"
              :style="{ width: '100%' }"
              clearable></el-input>
            <!-- <el-select
          ...
          <el-form-item v-if="activeData.__config__.fileSize !== undefined" label="文件大小">
            <el-input v-model.number="activeData.__config__.fileSize" type="number" min="0" placeholder="请输入文件大小">
              <el-select
                slot="append"
          ...
          <el-form-item v-if="activeData['list-type'] !== undefined" label="列表类型">
            <!-- <el-radio-group v-model="activeData['list-type']" size="small">
              <el-radio-button label="text">secret</el-radio-button>
            ...
            <el-select
                ...
                clearable
              >
                <el-option label="密级" value="secret" />
                <el-option label="text" value="text" />
                <el-option label="picture" value="picture" />
                <el-option label="picture-card" value="picture-card" />
              </el-select>
          </el-form-item>
          <el-form-item
          ...
          <template
            v-if="['el-checkbox-group', 'el-radio-group', 'el-select', 'tree-select'].indexOf(activeData.__config__.tag) > -1"
          >
            <el-divider>选项</el-divider>
            <el-form-item label="数据类型">
              ...
              <span class="view-fields-link" title="查看出参" @click="viewFields">
                <i class="el-icon-link" />
              </span>
            </el-form-item>
            ...
            <template v-if="activeData.__config__.dataType === 'dynamic'">
              <el-form-item label="选项组件">
                <el-select v-model="activeData.__config__.selectComp" placeholder="请选择选项组件" clearable @change="changeSelectComp">
                  <el-option v-for="item in selectCompOption" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
              <template v-if="selectCompParam.sqlParam && selectCompParam.sqlParam.length">
                <el-divider>SQL参数</el-divider>
                <el-form-item :label="item.paramName" v-for="item in selectCompParam.sqlParam" :key="item.paramName">
                  <el-input v-model="activeData.__config__.selectParam.sqlParam[item.paramName]" placeholder="请输入SQL参数" />
                </el-form-item>
              </template>
              <template v-if="selectCompParam.reportParam && selectCompParam.reportParam.length">
                <el-divider>报表参数</el-divider>
                <el-form-item :label="item.paramName" v-for="item in selectCompParam.reportParam" :key="item.paramName">
                  <el-input v-model="activeData.__config__.selectParam.reportParam[item.paramName]" placeholder="请输入报表参数" />
                </el-form-item>
              </template>
              <el-divider>自定义参数</el-divider>
              <div
                v-for="(item, index) in activeData.__config__.selectParam.param"
                :key="index"
                class="select-item"
              >
                ...
                <el-input
                  ...
                  size="small"
                  :value="item.value"
                  @input="setCustomParamValue(item, $event)"
                />
                <div
                  class="close-btn select-line-icon"
                  @click="activeData.__config__.selectParam.param.splice(index, 1)"
                >
                  <i class="el-icon-remove-outline" />
                </div>
              </div>
          ...
          <el-form-item v-if="activeData.hidden !== undefined" label="是否隐藏">
            <el-switch v-model="activeData.hidden" />
          </el-form-item>
            ...
            >
              <span slot-scope="{ node, data }">
                <span class="node-label">
                  <i :class="data.__config__?data.__config__.tagIcon:data.tagIcon"></i>
        ...
        <el-form v-show="currentTab === 'form'" size="small" label-width="90px">
          <el-form-item label="表单名">
            <el-input v-model="formConf.formRef" placeholder="请输入表单名（ref）" readonly/>
          </el-form-item>
          <el-form-item label="表单模型">
            <el-input v-model="formConf.formModel" placeholder="请输入数据模型" readonly/>
          </el-form-item>
          <el-form-item label="数据源">
            <el-select
              v-model="formConf.formDataBase"
              placeholder="请选择数据源"
              :style="{width: '100%'}"
          ...
          <el-form-item label="校验模型">
            <el-input v-model="formConf.formRules" placeholder="请输入校验模型" readonly/>
          </el-form-item>
    ...
    <icons-dialog
      ...
      @select="setIcon"
    />
    <common-dialog :visible="viewFieldsVisible" title="出参" :dialogConfig="dialogConfig" @close="viewFieldsClose" @handle-cancel="viewFieldsClose" @handle-ok="viewFieldsClose">
      <template #dialog>
        <el-divider>弹出选择出参</el-divider>
        <ul style="padding:0 10px;">
          <li style="padding:5px 0;" v-for="(item,index) in ejectSelectGinseng" :key="index">{{item}}</li>
        </ul>
        <el-divider>系统参数出参</el-divider>
        <ul style="padding:0 10px;">
          <li style="padding:5px 0;" v-for="(item,index) in systemParamsGinseng" :key="index">{{item}}</li>
        </ul>
        <el-divider>前置数据作为后置数据的检索条件</el-divider>
        <ul style="padding:0 10px;">
          <li>
          设置例子：<br>
          1.拖一个下拉选择组件作为前置；再拖一个下拉选择组件作为后置;<br>
          2.前置选一个数据源字段;<br>
          3.后置选一个数据源字段;<br>
          4.后置在右侧组件属性-选项-选择选项组件后，带出的参数里填写 '$'+前置的数据源字段。
          </li>

        </ul>
        <el-divider>计算器功能，目前只支持加法运算</el-divider>
        <ul style="padding:0 10px;">
          <li style="padding:5px 0;">
            设置例子：合计的变量中设置$COMPUTED.xxx+$COMPUTED.xxx<br>
            1.固定的前缀为$COMPUTED；<br>
            2.xxx为要进行加法运算的fields。
          </li>
        </ul>
      </template>
    </common-dialog>
...
<script>
import {
  Scrollbar, Tabs, TabPane, Form, FormItem, Select, Option, Input, Slider, InputNumber,
  RadioGroup, RadioButton, Divider, Button, Tree, ColorPicker, Switch
} from 'element-ui'
...
export default {
  ...
  data () {
    return {
      ...
      dataBaseOption: [], // 数据源
      formFieldsOption: [], // 数据源字段
      childrenTableOption: [], // 子表表名
      childrenFieldsOption: [], // 子表字段
      selectCompOption: [], // 选项组件下拉option
      selectCompParam: {}, // 下拉选项组件的参数
      ejectSelectColumns: [], // 根据弹出框选项组件获取报表列信息
      viewFieldsVisible: false, // 关联数据信息是否可见
      dialogConfig: {
        modal: false
      },
      ejectSelectGinseng: [], // 弹出选择出参ejectSelectGinseng
      systemParamsGinseng: [] // 系统参数出参systemParamsGinseng
    }
  },
  mounted () {
    let that = this
    this.$api['formGenerator.getFormDataBase']().then(function (res) {
      if (res && res.length) {
        that.dataBaseOption = res.map(item => {
          return {
    ...
    if (this.formConf.formDataBase) {
      this.getFormFields(this.formConf.formDataBase)
      this.getChildTable(this.formConf.formDataBase)
    }
    this.systemParamsGinseng = Object.keys(this.$store.state.user.userInfo).map(item => {
      return '$SYSTEM_PARAMS_SELECT.' + item
    })
  },
  computed: {
    documentLink () {
  ...
  watch: {
    formConf: {
      handler (val) {
        this.getFormFields(val.formDataBase)
        this.getChildTable(val.formDataBase)
      },
      deep: true
    },
    drawingList: {
          ...
          val.map(item => {
            if (item.__config__.layout === 'masterSlaveTable') {
              ...
              if (item.__config__.children.length) {
                // 如果表单组件为子表中的组件，则默认为子表中的表单组件加上子表的表名childrenTable
                item.__config__.children.map(childItem => {
                  childItem.__config__.childrenTable = item.__config__.childrenTable
                })
              }
            }
          })
    ...
    activeData: {
      handler (val) {
        if (val.__config__.tag === 'el-select' || val.__config__.tag === 'el-radio-group' || val.__config__.tag === 'el-checkbox-group') {
          this.getSelectComp({ selectionType: '1' })
        } else if (val.__config__.tag === 'eject-select') {
          this.getSelectComp({ selectionType: '3' })
          this.changeEjectSelectComp(val.__config__.selectComp)
        } else if (val.__config__.tag === 'tree-select') {
          this.getSelectComp({ selectionType: '2' })
        }

        // 监听所选组件，获取组件所要绑定的字段（切换主表字段与多个子表字段）
        if (val.__config__.selectChildTable && val.__config__.childrenTable) {
          this.getChildrenFields(val.__config__.childrenTable)
        }
      },
      deep: true
    }
  ...
  methods: {
    ...
    // 添加自定义参数
    addCusotmParamItem () {
      this.activeData.__config__.selectParam.param.push({
        label: '',
        value: ''
      })
    },
    ...
    dataBaseChange (val) {
      this.formConf.formDataBase = val
    },
    formFieldsChange (val) {
      this.$set(this.activeData.__config__, 'formFields', val)
    },
    changeRenderKey () {
    ...
    getFormFields (param) {
      let that = this
      this.$api['formGenerator.getFormFields']({ id: param }).then(function (res) {
        if (res && res.length) {
          that.formFieldsOption = res.map(item => {
            return {
              label: item.label,
              value: item.value
            }
    ...
    getChildTable (param) {
      let that = this
      this.$api['formGenerator.getChildTable']({ id: param }).then(function (res) {
        if (res && res.length) {
          that.childrenTableOption = res.map(item => {
            return {
              label: item.label,
              value: item.value
            }
    ...
    // 选择子表
    childrenTableChange (val) {
      this.getChildrenFields(val)
    },
    // 获取子表字段
    getChildrenFields (param) {
      let that = this
      this.$api['formGenerator.getFormFields']({ id: param }).then(function (res) {
        if (res && res.length) {
          that.childrenFieldsOption = res.map(item => {
            return {
              label: item.label,
              value: item.value
            }
    ...
    getSelectComp (param) {
      let _this = this
      this.$api['formGenerator.getSelectionOption'](param).then(res => {
        _this.selectCompOption = res
      })
    },
    // 获取选中的选项组件参数
    changeSelectComp (val) {
      let _this = this
      this.$api['formGenerator.getSelectionConfig']({ id: val }).then(res => {
        console.log(res, 'res---param')
        _this.selectCompParam = res
      })
    },
    // 根据选中的弹出选项组件获取报表列信息
    changeEjectSelectComp (val) {
      if (val) {
        let _this = this
        this.$api['formGenerator.getSqlViewAllColumnBySelectionId']({ id: val }).then(res => {
          console.log(res, 'res---param')
          _this.ejectSelectColumns = res
        })
        this.$api['formGenerator.getSelectionDetail']({ id: val }).then(res => {
          console.log(res, 'res---getSelectionDetail')
          if (res) {
            this.activeData.__config__.reportCode = res.reportCode
          } else {
            this.activeData.__config__.reportCode = ''
          }
        })
      }
    },
    // 根据选中的弹出选项组件获取报表列信息，与弹出框的数据源字段拼接组成关联数据信息：viewField的值
    viewFields () {
      this.viewFieldsVisible = true
      if (this.ejectSelectColumns.length && this.activeData.__config__.tag === 'eject-select') {
        let _this = this
        this.ejectSelectGinseng = this.ejectSelectColumns.map(item => {
          return '$' + _this.activeData.__vModel__ + '_SELECT.' + item.label
        })
      }
    },
    viewFieldsClose () {
      this.ejectSelectGinseng = []
