...
const platform = {
  ...
  actions: {
    ...
    async setTheme ({ commit, state }, { theme, handler = false }) {
      ...
      //
      const themeCluster = theme && theme.includes('#') ? getClusterColor(theme.replace('#', '')) : []
      ...
      const updateThemeStyleText = (styleText, originalCluster, themeCluster) => {
        ...
        originalCluster.forEach((color, index) => {
          newStyleText = themeCluster.length && newStyleText.replace(new RegExp(color, 'ig'), themeCluster[index])
        })
        return newStyleText
      }
