...
function searchFilter (parent, searchForm, ganttObject) {
  if (JSON.stringify(searchForm) !== '{}') {
    ...
    if (wbsCheck && deepCheck && taskNameCheck && startDateCheck && managerStatusCheck && endDateCheck && productQuantityCheck && userNameCheck && deptNameCheck && dateCheck && statusCheck && monitorsCheck && planTypeCheck) {
      return true
    }
...
GanttCourtyardObject.setOnBeforeEditStart = function (ganttObject, vueThis) {
  return ganttObject.ext.inlineEditors.attachEvent('onBeforeEditStart', function (state) {
    ...
    console.log(task.dutyUserId, task.nowUserId, 'task.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserIdtask.dutyUserId === task.nowUserId')
