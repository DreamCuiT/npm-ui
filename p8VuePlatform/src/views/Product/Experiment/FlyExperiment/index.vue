...
export default {
  ...
  data () {
    ...
    return {
      ...
      tableApi: 'experimentManager.flyExperimentList',
      columns: columns,
      tableHeight: 1,
      comp: this
  ...
  methods: {
    ...
    createFlyExperiment () {
      this.experimentId = this.id
      this.rowId = ''
      this.title = '新建试验信息'
      this.visibleProblemEdit = true
    },
    updateFlyExperiment (record) {
      this.experimentId = record.id
      this.title = '修改试验信息'
      this.visibleProblemEdit = true
    },
    removeFlyExperiment (record) {
      let that = this
      that.$confirm(`是否要删除该条数据？`, '提示', {
        ...
        that.$api['experimentManager.deleteFlyExperiment']({ id: record.id }).then(res => {
