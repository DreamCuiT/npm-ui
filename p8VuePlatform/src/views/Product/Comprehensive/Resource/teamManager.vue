<template>
  <div style="height: 100%">
    <center-right-layout>
      <template #left>
          <normal-layout :westFullHeight="true">
            <template #north>
            ...
            <template #west>
              <div class="role-con">
                ...
                <vue-perfect-scrollbar class="role-list">
                  <ul class="fixed-role">
                    <li :class="item.roleId===rolesSelectedId?'active':''" v-for="item in fixedRoles" :key="item.id" @click="rolesHandle(item)">
                      <i class="el-icon-s-custom"></i>
                      <edit-input :textValue="item.name" :record="item"></edit-input>
                    </li>
                  </ul>
                  <div class="general-role">
                    <li :class="item.id===rolesSelectedId?'active':''" v-for="(item,index) in generalRoles" :key="item.id" @click="rolesHandle(item)">
                      <i class="el-icon-s-custom"></i>
                      <edit-input :textValue="item.name || item.roleName" :record="item" :iconShow="true" @delete="deleteRolesHandle(index)" @onChange="changeRolesHandle"></edit-input>
                    </li>
                  </div>
            ...
            <template #center>
              <div class="table-con">
                <div class="add-member">
                  <el-button type="text" icon="el-icon-circle-plus" @click="addMemberHandle">添加人员</el-button>
                </div>
    ...
    <dialog-select-member
      :visibleDislogMember="visibleDialogMember" :loginFlag="loginFlag"  :selectRoleId="selectRoleId"
      ...
      this.$api['projectTeam.index']({ wholeDescribeId: this.id }).then(res => {
        if (res) {
          ...
          if (this.fixedRoles.length) {
            ...
            this.selectRoleId = this.fixedRoles[0].roleId
            console.log(this.selectRoleId, this.fixedRoles[0].roleId, '111111')
