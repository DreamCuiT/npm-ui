  ...
  methods: {
      ...
      this.$api['planGanttManager.getDependenceByTaskId']({ taskId: taskId }).then(res => {
        ...
        if (res && res.ganttLinkResponse.length > 0) {
          res.ganttLinkResponse.forEach(function (item) {
            console.log(that.vueThis, 'dependenceEditworkTime')
            item.lag = item.lag / that.vueThis.workTime
            datas.push(item)
          })
    ...
    formSubmit (params) { // 单个form提交回调
      ...
      if (that.ganttName && that.ganttName === 'changeGantt') {
        ...
        console.log(that.vueThis, 'workTime122121')
        console.log(that.vueThis.workTime, 'workTime122121')
        that.$api['planGanttManager.saveDependence']({ id: delLink.id, target: that.taskId, source: delLink.source, type: delLink.type, lag: delLink.lag * that.vueThis.workTime }).then(function (res) {
          if (res) {
            thisDp.ignore(function () {
              if (delLink.id) {
                let link = ganttObject.getLink(delLink.id)
    ...
    sourceSelectRules () {
      ...
      if (this.ganttLinkResponse && this.ganttLinkResponse.length) {
        ...
        let sourceParents = []
        let currSourceLinkIdParents = ['17785182267ee7de7633c8abfbdb974e'] // 当前任务的前置 初始为前置父, 判断时会push下面子的id
        let currSourceLinkChilds = [] // 当前任务的前置的子任务id
