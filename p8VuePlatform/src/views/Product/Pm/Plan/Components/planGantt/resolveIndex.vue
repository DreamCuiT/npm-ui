...
<!--              class="detail_span">{{selectTaskName}}-->
<!--        </span>-->
...
// 引入计划分解的planGantt
import { planResolveGantt } from '@/assets/commonJS/ganttJS/resolve/planResolveGanttObject'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      selectedTasks: [],
      buttonDatas: CommandResolveButtonData,
      resourceConfig: {},
      resourceTemplates: {},
      selectTaskId: '',
      startDate: '',
  ...
  watch: {
    ...
    planInfoId: function (newVal, oldVal) {
      if (newVal) {
        ...
        this.callParentSelectTasks()
        // cell编辑器打开前逻辑设置
        GanttResolveObject.setOnBeforeEditStart(myGantt, this)
        // 设置保存时机
        GanttResolveObject.setCellSaveConfig(myGantt)
        // 编辑器保存前校验
        GanttResolveObject.checkoutBeforeCellSave(myGantt, this)
        // 编辑器保存后逻辑
        GanttResolveObject.onSaveCellEven(myGantt, this)
      }
    },
  ...
  mounted () {
    console.log('载入resolveIndex planResolveGantt 页面')
    this.scrollBarHeight = 40 * this.menuData.length + 1 + 'px'
  },
  computed: {
    isDisable () {
  ...
  methods: {
    ...
    initGantt (planInfoId, viewType) {
      ...
      // 清空原有数据
      myGantt = GanttResolveObject.getGanttObject(vueThis.ganttName)
      if (myGantt) {
        GanttResolveObject.setGanttObject(vueThis.ganttName, {})
        myGantt.destructor()
      }
      // vuex存放
      vueThis.$store.dispatch('setVueThis', vueThis)
      // 初始化对象
      myGantt = planResolveGantt(vueThis.ganttName, vueThis)
      ...
      if (viewType === 'resource') {
        ...
        myGantt.ext.zoom.init(GanttResolveObject.zoomConfig(myGantt))
        // 资源负载图列头定义
        let resourceConfig = GanttResolveObject.resourceConfig(myGantt, vueThis)
        // 定义资源模板
        let resourceTemplates = GanttResolveObject.resourceTemplates(myGantt)
        // 重新布局
        myGantt.config.layout = GanttResolveObject.layout3(resourceConfig, resourceTemplates)
        // 样式绑定
        myGantt.templates.grid_row_class = GanttResolveObject.grid_row_class(myGantt)
        myGantt.templates.task_row_class = GanttResolveObject.task_row_class(myGantt)
        myGantt.templates.timeline_cell_class = GanttResolveObject.timeline_cell_class(myGantt)
        myGantt.templates.resource_cell_class = GanttResolveObject.resource_cell_class()
        myGantt.templates.resource_cell_value = GanttResolveObject.resource_cell_value()
      } else if (viewType === 'gantt') {
        myGantt.ext.zoom.init(GanttResolveObject.zoomConfig(myGantt))
        myGantt.config.layout = GanttResolveObject.layout1
      } else {
        myGantt.config.layout = GanttResolveObject.layout2
      }
      // 清空表头查询缓存
      vueThis.searchForm = {}
      // cell编辑器打开前逻辑设置
      GanttResolveObject.setOnBeforeEditStart(myGantt, vueThis)
      // 设置保存时机
      GanttResolveObject.setCellSaveConfig(myGantt)
      // 编辑器保存前校验
      GanttResolveObject.checkoutBeforeCellSave(myGantt, vueThis)
      // 编辑器保存后逻辑
      GanttResolveObject.onSaveCellEven(myGantt, vueThis)
      // 渲染对象
    ...
    loadGanttData (planInfoId, taskId, createPage) {
      let vueThis = this
      vueThis.$api['planGanttManager.loadPlanGanttData']({ planInfoId: planInfoId, dicType: 'ACTIVITY_TYPE', taskId: taskId, createPage: createPage, currentPage: 'overallPlanning' }).then(function (res) {
        if (res) {
          // 初始化数据
          let datas = {
            tasks: res.tasks,
