<template>
    <list-layout :headerVisible="headerVisible">
            ...
            <div id='table-contain'>
                ...
                <common-table ref='table'
                    ...
                    <template #tableSettings="{scope}">
                        <el-button v-if="scope.row.recordType === 'plan'" type="text"
                                @click="associationYplan(scope.row)">关联</el-button>
                    </template>
                    <template #dicName="{scope}">
        ...
        <template #drawer-panel>
            ...
            <common-drawer v-if="isAssociationYplanView"
                           :visible="isAssociationYplanView"
                           placement="top"
                           direction="ttb"
                           :drawerConfig="drawerConfig"
                           size="100%"
                           :isNeedCustomDrawerClass="false"
                           @close="closeAssociationYplan">
                <template #drawer>
                    <selectAssociationYplan :planInfoId = "planInfoId" @approval-close="approvalClose" @close="closeAssociationYplan"></selectAssociationYplan>
                </template>
            </common-drawer>
...
<script>
    ...
    import selectAssociationYplan from './connectCourtyardPlan/selectAssociationYplan'
    import GanttIndex from './connectCourtyardPlan/index'
    ...
    export default {
        ...
        data () {
            const columns = [
                {
                    ...
                    //   if (row.leaf && this.compType !== 'approvalHistoryView') {
                    //     return true
                    //   } else {
                    //     return false
                    //   }
            ...
            return {
                ...
                defaultMenu: {},
                drawerTitle: '加锁/解锁',
                queryParam: { taskId: this.taskId, businessId: this.businessId, planInfoIds: this.planInfoIds, filterTemplateData: this.filterTemplateData, compType: this.compType },
        ...
        methods: {
            ...
            tableRefresh (param) {
                ...
                param.then((res) => {
                    if (this.createPlanId) {
                     ...
                     if (this.pageType && this.pageType === 'approval') {
                            let arr = this.cheackData.map(row => { return row.id })
                            res.records.map(item => {
            ...
            closeSelectApproveUser () {
                ...
                this.$emit('approved', this.formData.taskId ? this.formData.taskId : '')
            },
            closeAssociationYplan () {
                this.isAssociationYplanView = false
            ...
            associationYplan (record) {
                this.yplanId = record.yplanId
                if (this.yplanId) {
                    this.drawerTitle = '院计划关联'
                    this.planInfoId = record.id
                    this.yplanConnect = true
                } else {
                    this.drawerTitle = '查看可关联的院计划'
                    this.planInfoId = record.id
                    this.isAssociationYplanView = true
                }
                this.cheackData = this.$refs.table.$refs.table.selection
            },
