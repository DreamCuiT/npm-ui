            ...
            <template slot-scope="scope">
              <template v-if="item.prop == 'surplusDate'">
                <template v-if="scope.row.flag> 0">
                  <span style="color: #59A869">剩余 {{ scope.row[item.prop] }} 天</span>
                </template>
                <template v-else-if="scope.row.flag == 0">
                  <span style="color: #59A869">今天</span>
                </template>
                <template v-else>
                  <span style="color: #EB3941">超期 {{ scope.row[item.prop] }} 天</span>
                </template>
              </template>
              <template v-else-if="item.prop == 'monitorpoint'">
...
export default {
  ...
  methods: {
    ...
    search (param) {
      let newParams = { ...param, ...(this.queryParam.productName ? { productName: this.queryParam.productName } : {}), ...(this.queryParam.productName ? { productName: this.queryParam.productName } : {}), ...(this.queryParam.productName ? { productName: this.queryParam.productName } : {}) }
