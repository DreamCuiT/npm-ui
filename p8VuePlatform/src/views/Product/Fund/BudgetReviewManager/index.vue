  ...
  <list-layout>
    <template #north>
      <el-button v-if="isCanEdit()"
                 type="primary"
                 round
                 @click="submitComeIn()">发布</el-button>
      <!-- <el-button v-if="isCanChange()"
                 type="primary"
                 round
                 @click="submitComeIn()">预算变更</el-button> -->
    </template>
    <template #center>
      <div class="tableDiv">
...
export default {
  ...
  methods: {
    ...
    initData () {
      if (this.$route.name === 'budgetReviewManager') {
        this.initColumns()
        this.queryList()
      }
    },
    ...
    isCanEdit () {
      return this.revenueStatus === '4521'
    },
    isCanChange () {
      return false
    ...
    submitComeIn () {
      ...
      that.$confirm(`确认要发布预算吗?`, '提示', {
        ...
        that.$api['collectionBudget.updateProjectRevenueInfoComeIn']({ ...this.thirdMenuParam, flag: 'release' }).then(res => {
          if (res && res === 'true') {
