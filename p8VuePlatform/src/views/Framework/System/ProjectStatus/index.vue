  ...
  <list-layout>
    <template #center>
            ...
            <el-table-column prop="icon"
              ...
              <template slot-scope="scope">
                <i :class="scope.row.icon"
                   :style="{'font-size':'20px','color':scope.row.color}"></i>
              </template>
            </el-table-column>
      ...
      <common-dialog title="图标选择"
        ...
        <template #dialog>
          <icon-selector @icon-select="onEditIConOK"
                         :selectedName="record.icon"
                         :selectedColor="record.color"
                         :colorPicker=true></icon-selector>
        </template>
      </common-dialog>
    </template>
  </list-layout>
...
export default {
  ...
  methods: {
    editIcon (row, column, event) {
      this.record = row
      this.iconTemp = row.icon
      this.colorTemp = row.color
    ...
    getStatusList (kTeamId) {
      let that = this
      let statusKey = []
      that.$api[that.tableApi]({ dicType: 'STAND_PROJECT_STATUS' }).then(function (res) {
        let dataTemp = []
        res.forEach(function (item) {
          let temp = {}
          temp.id = item.value
          temp.name = item.label
          temp.iconKey = 'projectStatus-icon-' + item.value
          temp.colorKey = 'projectStatus-color-' + item.value
          dataTemp.push(temp)
          statusKey.push('projectStatus-icon-' + item.value)
          statusKey.push('projectStatus-color-' + item.value)
        })
