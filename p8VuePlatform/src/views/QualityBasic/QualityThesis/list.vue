...
<script>
    ...
    export default {
            ...
            const columns = [
                ...
                },
                {
                    ...
                    formatter: function (row) {
                        if (row.pcollectionUnitName) {
                            return row.pcollectionUnitName + '-' + row.collectionUnitName
                        } else {
                            return row.collectionUnitName
                        }
                    },
                    minWidth: 180,
                    align: 'center'
        ...
        methods: {
            ...
            createThesis () {
                this.drawerTitle = '新建论文'
                this.drawerVisible = true
                this.record = {}
            },
            updateThesis (record) {
                this.drawerTitle = '修改论文'
                this.drawerVisible = true
                this.record = record
            },
            removeThesis (record) {
                let that = this
                this.$confirm('是否确定要删除该论文？', '提示', {
                    confirmButtonText: '是',
                    cancelButtonText: '否',
                    type: 'warning'
                }).then(() => {
                    that.$api['QualityThesis.remove']({ id: record.id }).then(res => {
                        if (res && res.data) {
                            that.$message({
