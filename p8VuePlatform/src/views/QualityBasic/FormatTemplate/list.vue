...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            createTemplate () {
                this.drawerTitle = '新建格式模板'
                this.drawerVisible = true
                this.record = {}
            },
            updateTemplate (record) {
                this.drawerTitle = '修改格式模板'
                this.drawerVisible = true
                this.record = record
            },
            removeTemplate (record) {
                let that = this
                this.$confirm('是否确定要删除该格式模板？', '提示', {
                    confirmButtonText: '是',
                    cancelButtonText: '否',
                    type: 'warning'
                }).then(() => {
                    that.$api['FormatTemplate.remove']({ id: record.id }).then(res => {
                        if (res && res.data) {
                            that.$message({
