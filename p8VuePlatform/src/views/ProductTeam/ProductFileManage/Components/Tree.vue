<template>
    <div>
        <v-contextmenu ref="treeMenu">
            <v-contextmenu-item  @click="createOrg">新建菜单</v-contextmenu-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            initOrgTree () {
                this.$api['productFileManage.productFileTree']().then(res => {
                    this.orgTreeData = generateTree(res)
                })
            },
            qualityTree () {
                let that = this
                that.$api['productFileManage.productFileTree']().then(res => {
            ...
            deleteOrg () {
                ...
                if (id) {
                    this.$confirm(`此操作将删除:[${name}], 确认要删除吗?`, '提示', {
                        ...
                        this.$api['productFileManage.removeNode']({ id: id }).then(res => {
                            if (res && res.data) {
