    ...
    <normal-layout>
        ...
        <template #drawer-panel>
            <common-drawer v-if='visibleCreateFileManageDrawer'
                ...
                <template #drawer>
                    <file-manage-edit @save-success='saveCallback' :clickedNodeData="clickedNodeData"></file-manage-edit>
                </template>
            </common-drawer>
        </template>
    </normal-layout>
    ...
    export default {
        ...
        data () {
            return {
                ...
                currFileManageId: '',
                treeApi: 'productFileManage.productFileTree',
                queryParam: {},
                treeParams: {},
                tableApi: 'productFileManage.getFileList',
                advanced: false,
                columns: columns,
                tableHeight: 0,
        ...
        methods: {
            ...
            removeFile (record) {
                ...
                that.$confirm(`是否要删除该计划？`, '提示', {
                    ...
                    let type = 'success'
                    let msg = '删除成功！'
                    that.$api['productFileManage.remove']({ id: record.id }).then(res => {
                        if (res && res.data) {
                            that.$message({
            ...
            reSet () {
                let productFileId
                if (this.queryParam.productFileId) {
                    productFileId = this.queryParam.productFileId
                }
                let that = this
                Object.keys(that.queryParam).forEach(key => { that.queryParam[key] = '' })
                if (productFileId) {
                    this.queryParam.productFileId = productFileId
                }
                Vue.nextTick(
                    function () {
                        that.$refs.table.searchData()
            ...
            // tree
            onSelect (data) {
                this.queryParam.productFileId = data.id
