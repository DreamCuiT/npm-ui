<template>
    <div>
        <v-contextmenu ref="treeMenu">
            <v-contextmenu-item  @click="createOrg">新建组织机构</v-contextmenu-item>
            <v-contextmenu-item v-if="contextmenuSelectedNode != null && contextmenuSelectedNode.id != 1"
                                @click="deleteOrg">删除组织机构</v-contextmenu-item>
        </v-contextmenu>
...
<script>
    ...
    export default {
        ...
        methods: {
            initOrgTree () {
                this.$api['productOrganization.organizationTree']().then(res => {
                    this.orgTreeData = generateTree(res)
                })
            },
            qualityTree () {
                let that = this
                that.$api['productOrganization.organizationTree']().then(res => {
            ...
            deleteOrg () {
                ...
                if (id) {
                    this.$confirm(`此操作将删除:[${name}], 确认要删除吗?`, '提示', {
                        ...
                        this.$api['productOrganization.removeNode']({ id: id }).then(res => {
                            if (res && res.data) {
