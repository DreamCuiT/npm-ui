<template>
    <div>
        <v-contextmenu ref="treeMenu">
            <v-contextmenu-item v-if="orgTreeData[0].pId == null" @click="createOrg">新建</v-contextmenu-item>
            <v-contextmenu-item  @click="editOrg">修改</v-contextmenu-item>
            <v-contextmenu-item  @click="deleteOrg">删除</v-contextmenu-item>
        </v-contextmenu>
...
<script>
    ...
    export default {
        ...
        methods: {
            initOrgTree () {
                this.$api['ExpertDatabaseInformation.productTeamTree']().then(res => {
                    this.orgTreeData = generateTree(res)
                })
            },
            qualityTree () {
                let that = this
                that.$api['ExpertDatabaseInformation.productTeamTree']().then(res => {
            ...
            deleteOrg () {
                ...
                if (id) {
                    this.$confirm(`此操作将删除:[${name}], 确认要删除吗?`, '提示', {
                        ...
                        this.$api['ExpertDatabaseInformation.productTeamRemoveNode']({ id: id }).then(res => {
                            if (res && res.data) {
