<template>
    <div>
        <v-contextmenu ref="treeMenu">
            <v-contextmenu-item v-if="orgTreeData[0].pId == null" @click="createOrg">新建
            </v-contextmenu-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            initOrgTree () {
                this.$api['TypeSpectrum.typeSpectrumTree']().then(res => {
            ...
            deleteOrg () {
                ...
                if (id) {
                    this.$confirm(`此操作将删除:[${name}], 确认要删除吗?`, '提示', {
                        ...
                        this.$api['TypeSpectrum.typeSpectrumRemoveNode']({ id: id }).then(res => {
                            if (res && res.data) {
